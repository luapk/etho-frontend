...
function AudioWaveform({ events = [], duration = 30, currentTime = 0, isPlaying = false, onSeek, videoContext = '' }) {
  ...
  const eventRanges = useMemo(() => {
    ...
    return events.map(e => ({
      ...
      label: getContextualDescription(e, videoContext),
      interpretation: e.interpretation,
      zone: getVocalizationZone(e.type, e.subtype),
      loudness: e.subtype?.toLowerCase().includes('aggressive') ? 1.0 :
    ...
    <div className="rounded-xl p-5" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
      ...
      {events && events.length > 0 && (
        <div className="mt-4 space-y-2">
          {events.slice(0, 5).map((event, idx) => {
            const zone = getVocalizationZone(event.type, event.subtype);
            const colors = ZONE_COLORS[zone];
            const description = getContextualDescription(event, videoContext);
      ...
      {(!events || events.length === 0) && (
        <p className="text-white/40 text-sm text-center mt-4 font-roboto">
          No pet vocalizations detected in this video
        </p>
      )}
